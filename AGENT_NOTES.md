...
- Payload support was removed from the CPU MuJoCo scenario hierarchy. `ObstacleStreetScenario` and `BridgeScenario` now inherit `BaseScenario` directly, `global_obs` stays empty for these scenarios, and env settings should not carry `payload_type` anymore.
