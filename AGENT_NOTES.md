...
- MJW training runs use a lazily created, persistent second environment for frozen milestone evaluations. The defaults are 256 evaluation lanes, one accepted episode per lane, deterministic actions, milestones at 25/50/75/90/95/100%, and five exact-state evaluation recordings under `videos/eval`. Evaluation copies and freezes current normalization statistics, isolates Torch/CUDA RNG and action-distribution temporal state, and forces a seeded settled full reset at every milestone. Results are written only to `eval_log.csv`; they are not merged into training `log.csv`. `evaluation_num_envs` and `evaluation_episodes_per_env` trade parallelism for repeated episodes per lane.
