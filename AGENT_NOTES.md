...
- Obstacle-street progress reward has a category multiplier plus components: `progress_reward_weight` scales the whole progress category, while `forward_reward_weight` only scales the forward-distance component. Wall presets keep the old forward scale as `forward_reward_weight=0.2` in `WALL_PASS_KWARGS`.
...
- `BaseScenario` no longer owns a generic `compute_progress(...)` helper. It only requires `compute_progress_reward(...)`, and each scenario owns both its reset-time progress baseline and its per-step reward delta logic. Shared payload/swarm y-progress baseline math now lives in `PayloadScenario._compute_progress_baseline(...)`.
