...
- PyTorch-only xLSTM cell/adapters live in `swarmbots.learn.algos.xlstm`: `MLSTMTemporalSequenceModel` and `SLSTMTemporalSequenceModel` already match the `TemporalSequenceModel` contract, including `valid_mask` state preservation and `reset_mask` zero-before-step semantics. They are not wired into RMAT configs by default yet.
