...
- `SAC(learning_rate_warmup_updates=...)` warms actor/critic optimizer LRs only; the entropy coefficient optimizer stays at the base `learning_rate`.
- SAC accepts `auto_<value>` and `auto*<value>` for entropy coefficient/target suffixes; `target_entropy` scales the per-active-agent action-dim target. Current MJW TMASAC defaults use `ent_coef="auto_0.5"` and `target_entropy="auto_0.5"`; `auto`/alpha 1.0 makes the initial entropy term dominate Q targets for large joint action spaces.
- SAC actor updates include action-dist extra losses through the loss-only path when available, reduced like summed agent log-probs. For TMASAC RSMK, MJW experiments make that extra entropy categorical-only (`kumaraswamy_ent_scale=0.0`) with categorical `entropy_floor=0.35` to discourage sign collapse without explicitly widening magnitudes; the SAC loss-only path skips expensive Kumaraswamy entropy/metric computation in that case. `EntropyLossConfig.max_entropy` is deprecated alias for `entropy_floor`.
...
- `experiments/mjw_experiment_common.py` supports `policy_variant="tmasac"` through an SAC branch; TMASAC NOP is configured with `SACNOPConfig`, not `NextObsPredWrapper`. `SACNOPConfig` defaults to disabled; enable it only with explicit next-observation prediction targets.
...
- `TMASACPolicy` uses separate actor/critic MAT encoders by default. Optional shared observation encoding is policy-owned via `shared_encoder_config` (or legacy `share_observation_encoder=True`); actor reads detach shared latents, while critic/NOP critic gradients update the shared encoder and its target copy is Polyak-updated with the critic target. The critic action-coembeds local obs/shared latents, hidden local vars, and actions before its transformer. SAC NOP modules are selected by latent source (`critic`, `actor`, `both`, or `shared_encoder`); `both` means separate actor/critic projections, transition models, and losses. Multi-step SAC NOP uses `SAC(nop_steps=4)` by default and samples a separate contiguous episode segment batch for NOP losses when NOP is enabled; if replay has no valid contiguous NOP window yet, SAC skips only the auxiliary NOP loss and still runs the main actor/critic update.
...
- The PO-wall medium continuous-connector TMASAC experiment is split into `experiments/mjw_po_wall_medium_1024x1_250M_tmasac_cont_conn_act` and intentionally uses `num_envs=1024`, `rollout_steps_per_env=1` so SAC trains after each single env step batch. Keep it separate from the PPO/MAT `1024x4` continuous-connector experiment.
