...
- `experiments/mjw_experiment_common.run_experiment` exposes MAT policy `enc_nhead`/`dec_nhead`, encoder-only `mat_encoder_transformer_ff_hidden_dims`, and binary connector `bernoulli_initial_prob` for experiment sweeps. MatDec only uses encoder heads; MatQCX uses both encoder and decoder heads. The NOP transition model head count stays on the runner default unless that path is changed separately.
