...
- The 1024x4 MJW ablation helper exposes repo-owned orthogonal init gains through `MATInitGains` and `NOPInitGains`; defaults preserve the old `0.01` gain. Transformer FF `linear1` gains default to `None` to preserve PyTorch's default initialization unless explicitly set, with `linear2` using gain `1.0` when explicit FF init is enabled. MAT observation/token projection gains default to `None` to preserve old all-layers gain behavior. MATOrig still uses its separate reference-style initializer.
