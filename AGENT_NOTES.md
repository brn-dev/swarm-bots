...
- `swarmbots/mjw_env`: MJWarp batched GPU env path. It still has a narrower scenario set than `mj_env`, but now covers both obstacle-street wall and payload-plane scenarios on the same batched env/runtime architecture; preconnected swarm pool only, quantized twists only, minimal contacts only, capsules only.
...
- Shared MJ/MJW scenario preset defaults live in `swarmbots/scenario_reward_kwargs.py`; keep mirrored common, wall, and payload reward kwargs there instead of duplicating them in `scenario_presets.py` / `mjw_scenario_presets.py`.
...
- Payload MAT/NOP training now has first-class entrypoints too: `scripts/run_mat_nop_payload.py` for CPU MuJoCo and `scripts/run_mat_nop_payload_mjw.py` for MJWarp. They mirror the canonical wall launchers but use the payload-plane presets and payload-specific run directories.
...
- Experiment entrypoints under `scripts/experiments/` should not import helpers from other script files. If a script-local helper is needed there, either move it into a proper package module or duplicate the small helper locally inside the experiment shared file. When run directly, these scripts should also add the repo root to `sys.path` for package imports and derive run/output paths from `__file__` instead of assuming `cwd == scripts/`.
