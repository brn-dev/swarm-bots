...
- MAT compile gotcha: pass `tgt_is_causal` explicitly into `nn.TransformerDecoder`; otherwise PyTorch tries to detect causal masks with a tensor `.all()`/`.item()` and Dynamo graph-breaks. Only mark the plain triangular masks causal (`FULL_AUTOREGRESSIVE` parallel/step masks and `CONTEXT_TOKENS_ONLY` step mask); the custom parallel masks are not causal.
...
- Shared MJ/MJW scenario preset kwargs live in `swarmbots/scenario_presets/scenario_presets_kwargs.py`; keep one complete kwargs dict per mirrored scenario (`*_SCENARIO_KWARGS`) plus `COMMON_SCENARIO_KWARGS`, instead of splitting reward kwargs from geometry/obs defaults or duplicating defaults in `scenario_presets.py` / `mjw_scenario_presets.py`. MJW-only kwargs like reward-kernel compilation still belong in `mjw_scenario_presets.py`.
...
- Generic grouped experiment result plots live in `plot_logs/experiment_results.py`. It expects `RUN_DIR/group/run/log.csv*`, labels only groups as `group (n=...)`, supports optional group order and an optional horizontal theoretical-maximum line, and writes high-DPI grouped plus individual `ep_rew_ema` PNGs only. Group curves use the union of run x-values and nan-aware reductions so the line continues after shorter runs finish with only the still-active runs contributing. The 4096 wall batch sweep wrapper is `experiments/mat_nop_wall_batch4096_env_sweep/plot_results.py`.
- The MJW wall batch/env sweep lives under `experiments/mat_nop_wall_mjw_batch_env_sweep/`. It mirrors `scripts/run_mat_nop_wall_mjw.py`, writes runs to `runs/mat_nop_swarm_bots_wall_mjw_batch_env_sweep/<variant>/<run_id>`, and currently has variants `1024x4`, `2048x4`, and `1024x8`.
...
- Payload and MoveTo MAT/NOP training now have first-class entrypoints too: `scripts/run_mat_nop_payload.py` / `scripts/run_mat_nop_move.py` for CPU MuJoCo and `scripts/run_mat_nop_payload_mjw.py` / `scripts/run_mat_nop_move_mjw.py` for MJWarp. They mirror the canonical wall launchers but use scenario-specific presets and run directories.
...
- Scheduled training recordings live in `swarmbots/utils/recording_schedule.py::install_scheduled_recordings`. It takes an explicit `{percentage: episode_count}` schedule with percentage keys in `0..100`; keep the zero-percent episode count in that mapping, not as a hook special case. Use `DEFAULT_RECORDING_SCHEDULE` for separate record-env flows and `DEFAULT_LIVE_RECORDING_SCHEDULE` for MJW live recording, where the final scheduled capture is before 100% so training does not exit immediately after starting it.
...
- MAT/NOP training launchers build run and checkpoint paths through `swarmbots.utils.run_paths.make_run_dir(...)`, anchored at the repo root. Keep new training scripts cwd-independent so `uv run python scripts/...` saves under `<repo>/runs/...`.
