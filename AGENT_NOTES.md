...
- Off-policy rollout/replay infrastructure lives in `swarmbots.learn.algos.off_policy`; `collect_off_policy_steps()` fills transition replay from a continuous vector-env stream and stores `terminations`, `truncations`, and optional `previous_actions`. `OffPolicyReplayBatch` does not expose `next_previous_actions`; use sampled `actions` when conditioning on `next_*` observations. Pass `rollout_device` to move the env wrapper, policy inference, and resumed rollout state explicitly; otherwise rollout follows the current env/state observation device. Policy rollouts use `act_with_temporal_state()` and keep `temporal_state` in `OffPolicyRolloutState`; random rollout state can later initialize temporal state when switching to a policy. gSDE policies must pass `gsde_reset_mode`; `OffPolicyRolloutState.gsde_noise_initialized` forces the first stochastic gSDE step to initialize full batch-shaped noise after random warmup, reset, or explicit rollout-device move. Shared SAME_STEP final-observation and episode-info handling lives in `swarmbots.learn.rollout_utils`.
- `OffPolicyReplayBuffer` can use pinned CPU float/bool data storage via `storage_pin_memory=True`; sampled tensors are pinned before CUDA transfer when needed, and `non_blocking_train_transfer` controls `.to(..., non_blocking=...)`. Advanced indexing still materializes sampled batch tensors, so pinning does not remove the gather copy.
- `OffPolicyReplayBuffer` is ring-backed with per-env transition slots and a per-env observation stream sized for `capacity + 1` slots so normal `next_obs` is shared with the next transition's current obs. SAME_STEP done transitions store terminal `final_obs` sparsely by `(env_idx, transition_slot)` and store the returned reset obs in the rolling stream. Keep transition-slot and observation-slot indexing separate when changing replay sampling. A nonempty replay buffer must keep collecting with the returned `OffPolicyRolloutState`; resetting the env into the same buffer without state breaks the observation-slot continuity. `add()` is a streaming append: after the first call, current obs should be the previous appended `next_obs`/reset obs, not a fresh copied row.
...
- PyTorch-only xLSTM memory-core adapters live in `swarmbots.learn.algos.xlstm`: `MLSTMTemporalSequenceModel` and `SLSTMTemporalSequenceModel` match the `TemporalSequenceModel` contract without replacing the full xLSTM block stack. RMAT uses one custom encoder stack (`RMATEncoder.encoder.layers`), not one nested `nn.TransformerEncoder` per block; each custom layer orders temporal and inter-agent-attention sublayers via `temporal_model_order`, can insert `inter_module_mlp`, and ends with the transformer feedforward MLP. `RMATEncoderConfig.temporal_residual` and `temporal_layer_norm` independently control transformer-style temporal skip and pre/post-norm wrapping; disable both for old recurrent baselines and do not duplicate that wrapper inside temporal cores. The default LSTM temporal core and temporal output projection are biasless so reset zero-input temporal rows do not emit a learned initial-state prior; other RMAT affine biases follow `RMATEncoderConfig.bias`. sLSTM keeps an output head norm. mLSTM uses a stabilized parallel sequence path with reset masks and padded/invalid rows folded into the parallel formula; invalid steps are state no-ops with zero output. `RMATEncoderConfig.temporal_model_cls/config` may be either a single spec or one spec per RMAT layer.
