...
- Medium/hard wall presets set `wall_success_threshold=1.0`, a separate per-wall y-offset from `wall_y` that terminates successfully once every active unit is strictly beyond every wall success threshold. It is not tied to `wall_pass_thresholds`; default/easy wall leave it disabled unless explicitly configured. Optional `wall_success_reward` is a flat terminal bonus on that same success step and is scaled by `progress_reward_weight` like the other obstacle-street progress terms.
