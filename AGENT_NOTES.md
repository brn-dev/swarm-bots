...
- Activation factories using `ParameterLearnMode.PER_FEATURE` require an explicit feature count to avoid forward-time parameter materialization under `torch.compile`; use the factory through `make_activation(...)` in generic `act_fn_cls` paths so MLP/transformer builders pass the correct feature width.
