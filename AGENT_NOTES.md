...
- SAC accepts `auto_<value>` and `auto*<value>` for entropy coefficient/target suffixes; `target_entropy` scales the per-active-agent action-dim target. MJW TMASAC experiments intentionally start with `ent_coef="auto_0.05"` and `target_entropy="auto_0.5"`; `auto`/alpha 1.0 makes the initial entropy term dominate Q targets for large joint action spaces.
- SAC actor updates include `action_dist.compute_extra_losses(...)`, reduced like summed agent log-probs. For TMASAC RSMK, MJW experiments make that extra entropy categorical-only (`kumaraswamy_ent_scale=0.0`) with categorical `entropy_floor=0.35` to discourage sign collapse without explicitly widening magnitudes. `EntropyLossConfig.max_entropy` is deprecated alias for `entropy_floor`.
