...
- Off-policy rollout/replay infrastructure lives in `swarmbots.learn.algos.off_policy`; `collect_off_policy_steps()` fills transition replay from a continuous vector-env stream and stores `terminations`, `truncations`, optional `previous_actions`, and optional `next_previous_actions`. Pass `rollout_device` to move the env wrapper, policy inference, and resumed rollout state explicitly; otherwise rollout follows the current env/state observation device. Shared SAME_STEP final-observation and episode-info handling lives in `swarmbots.learn.rollout_utils`.
