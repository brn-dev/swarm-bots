...
def test_po_wall_medium_samples_wall_y_into_hidden_global_obs_only_in_mj() -> None:
    ...
    sampled_wall_y_values: list[float] = []
    for _ in range(8):
        ...
        assert obs["hidden_global_vars"].shape == (3,)
        assert np.isclose(obs["hidden_global_vars"][0], state["wall_y"][0])
        sampled_wall_y_values.append(float(obs["hidden_global_vars"][0]))

    assert all(0.75 <= wall_y <= 1.25 for wall_y in sampled_wall_y_values)
    assert len({round(wall_y, 6) for wall_y in sampled_wall_y_values}) > 1
...
def test_po_wall_medium_mjw_space_keeps_wall_position_hidden() -> None:
    ...
    assert obs_space["hidden_global_vars"].shape == (3,)
...
def test_po_wall_medium_mjw_reset_sampling_puts_wall_y_in_hidden_global_vars() -> None:
    ...
    runtime.hidden_global_vars = torch.zeros((1, 3), dtype=torch.float32)
    runtime.wall_pass_absolute_thresholds = torch.zeros((1, 2), dtype=torch.float32)
    runtime._wall_thresholds_per_wall = 2
    ...
    assert hidden_global_vars.shape == (16, 3)
