...
def test_compiled_evaluation_refreshes_distribution_state_for_extra_losses() -> None:
    ...
    with (
        patch(
            "swarmbots.learn.algos.mat_orig.mat_orig_policy._ensure_torch_compile_available",
        ),
        patch(
            "swarmbots.learn.algos.mat_orig.mat_orig_policy.torch.compile",
        ...
        hidden_local_vars = torch.randn(
            batch_size,
            _DummyMATOrigEnv.n_agents,
            _DummyMATOrigEnv.hidden_local_vars_dim,
        )
        hidden_global_vars = torch.randn(batch_size, _DummyMATOrigEnv.hidden_global_vars_dim)
        agent_mask = torch.tensor([
            [True, True, True, False],
