...
from scripts.run_mat_nop_wall import (
    set_actuator_gsde_init_joint_stds,
    split_actuator_joints,
    wrap_vec_env,
)
...
from swarmbots.mjx_env import MjxSwarmBotsEnv
from swarmbots.mjx_env.mjx_gym_vector_env import MjxGymVectorEnv
from swarmbots.mjx_env.scenarios import mjx_scenario_presets
from swarmbots.mjx_env.scenarios.mjx_scenario_presets import mjx_default_wall
from swarmbots.mjx_env.swarm.mjx_homogeneous_swarm import MjxPreConnectedUnitLocationsConfig

MJX_IMPL = None
MJX_FRICTION = [1.0, 1e-2, 2e-4]


def make_preconnected_unit_start_locations(pool_seeds: tuple[int, ...] | None) -> MjxPreConnectedUnitLocationsConfig:
    return MjxPreConnectedUnitLocationsConfig(
        num_units=5,
        num_unit_probs={
            4: 1.0,
            5: 1.0,
...
def make_mjx_env(
    *,
    ...
    reset_pool_size: int,
    mjx_impl: str | None,
) -> tuple[MjxSwarmBotsEnv, MjxGymVectorEnv]:
    scenario = mjx_default_wall(
        first_wall_distance=1.0,
        unit_start_locations=unit_start_locations,
        reset_pool_size=reset_pool_size,
        mjx_impl=mjx_impl,
        friction=MJX_FRICTION,
        force_elliptic_cone=False,
    )
    base_env = MjxSwarmBotsEnv(
        scenario=scenario,
        episode_length=episode_length,
        action_repeat=15,
    )
    first_episode_lengths = np.maximum(
        1,
        (np.arange(1, n_envs + 1, dtype=np.float64) * episode_length / n_envs).astype(np.int32),
    )
    vector_env = MjxGymVectorEnv(
        env=base_env,
        num_envs=n_envs,
        first_episode_lengths=first_episode_lengths,
        seed=0,
    )
    return base_env, vector_env
...
def main() -> None:
    ...
    logger.info(f"{MJX_IMPL = }")

    if load_path is not None:
        if not load_path.endswith(".pt"):
            raise ValueError("load_path is missing .pt")
        run_id = load_path.split("/")[3]
        logger.info(f"{load_path = }")
    logger.info(f"{run_id = }")

    run_dir = f"../runs/mjx_mat_nop_swarm_bots_wall/{run_id}/"
    save_optimizer = True
    gamma = 0.99
    ...
    print("Creating MJX vector env...")
    base_env, vector_env = make_mjx_env(
        episode_length=episode_length,
        n_envs=n_envs,
        unit_start_locations=unit_start_locations,
        reset_pool_size=reset_pool_size,
    ...
    env_settings = base_env.get_settings()
    local_obs_dim = int(base_env.observation_space["local_obs"].shape[-1])
    global_obs_dim = int(base_env.observation_space["global_obs"].shape[-1])
    hidden_local_vars_dim = int(base_env.observation_space["hidden_local_vars"].shape[-1])
    hidden_global_vars_dim = int(base_env.observation_space["hidden_global_vars"].shape[-1])
    ...
    mat_policy = MATPolicy(
        ...
        config=MATPolicyConfig(
            ...
            critic_config=MATCriticConfig(
                ...
                use_popart=use_popart,
                popart_config=PopArtConfig(beta=popart_beta, init_sigma=popart_init_sigma),
            ),
            dropout=0.0,
            act_fn_cls=nn.GELU,
            ...
            bernoulli_config=BernoulliConfig(initial_prob=0.7, ent_loss_coef=1e-3),
            max_agents=20,
        ),
    )
    policy: BasePPOPolicy[Any, Any] = NextObsPredWrapper(
        policy=mat_policy,
    ...
    set_actuator_gsde_init_joint_stds(policy=policy, actuators_per_limb=actuators_per_limb, joint_stds=gsde_init_stds)
    print(policy)
    ...
    if isinstance(continuous_dist, StickyActionDist):
        ...
        logger.warning(f"Skipping act0_stickiness scheduler: action dist[0] is {type(continuous_dist)}")
    ...
    ppo = PPO(
        ...
        sampler_config=PPOWMSamplerConfig(batch_size=rollout_samples, num_next_steps=world_model_num_next_steps),
        n_epochs=6,
        gamma=gamma,
        gae_lambda=0.95,
        clip_range=0.07,
    ...
    ppo.learn(
        ...
        extra_run_metadata={
            ...
            "script_scenario_presets": Path(mjx_scenario_presets.__file__).read_text(encoding="utf-8"),
        },
        logging_console_keys=logging_console_keys,
        make_record_env=None,
    )
