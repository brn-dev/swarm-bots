...
from run_mat_qcs_nop_wall import wrap_vec_env, split_actuator_joints, set_actuator_gsde_init_joint_stds
...
def make_vector_env(
    *,
    ...
    return MJWSwarmBotsVectorEnv(
        scenario=default_wall(continuous_connector_actions=False),
        num_envs=num_envs,
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("run_mat_qcs_nop_wall_mjw.py requires CUDA.")
    ...
    run_dir = make_run_dir("mat_qcs_nop_swarm_bots_wall_mjw", run_id)
    save_optimizer = True
    ...
    run_with_discord_notification(
        run_name=f"mat_qcs_nop_wall_mjw/{run_id}",
        run_dir=run_dir,
        total_timesteps=total_timesteps,
        algorithm=ppo,
