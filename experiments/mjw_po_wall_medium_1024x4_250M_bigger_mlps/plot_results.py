...
EXPERIMENT_RUN_DIR = RUNS_DIR / "mjw_po_wall_medium_1024x4_250M_bigger_mlps"
...
DISPLAY_NAME_OVERRIDES = {
    baseline_group_name(group_name): f"{display_name} baseline"
    for group_name, display_name in BASE_DISPLAY_NAME_OVERRIDES.items()
} | {
    group_name: f"{display_name} bigger encoder MLPs"
    for group_name, display_name in BASE_DISPLAY_NAME_OVERRIDES.items()
}
EXTRA_GROUP_SOURCES = {
...
EXTRA_PLOT_SELECTIONS = tuple(
    ExperimentPlotSelection(
        ...
        title_suffix=(
            ...
            "Baseline vs Bigger Encoder MLPs"
        ),
