...
from swarmbots.learn.algos.mat_v2.mat_v2_decoder import MATv2Decoder, MATv2DecoderConfig
...
class MATv2PolicyConfig:
    encoder_config: MATEncoderConfig = field(default_factory=MATEncoderConfig)
    decoder_config: MATv2DecoderConfig = field(default_factory=MATv2DecoderConfig)
...
class MATv2Policy(BasePPOPolicy[PPOSamples]):

    def __init__(
            self,
            env: BaseLearnEnvWrapper,
            config: MATv2PolicyConfig = MATv2PolicyConfig(),
    ) -> None:
        super().__init__()
        self.config = config
