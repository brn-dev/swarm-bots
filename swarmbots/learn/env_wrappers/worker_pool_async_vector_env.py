...
__all__ = ["WorkerPoolAsyncVectorEnv", "AsyncState"]
...
class WorkerPoolAsyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        Warnings:
            ...
            from the code for ``_async_worker_pool`` and add changes.
        ...
        if requested_num_workers <= 0:
            raise ValueError(
                f"`num_workers` must be >= 1, got {requested_num_workers}."
            )
        self.num_workers = min(requested_num_workers, self.num_envs)
        ...
        if isinstance(observation_mode, tuple) and len(observation_mode) == 2:
            ...
            if observation_mode == "same":
                ...
                temp_envs = [env_fn() for env_fn in self.env_fns]
                try:
                    env_spaces = [env.observation_space for env in temp_envs]
                finally:
        ...
        target = worker or _async_worker_pool
        self.worker_env_indices = [[] for _ in range(self.num_workers)]
        self.env_to_worker_idx = [0 for _ in range(self.num_envs)]
        for env_idx in range(self.num_envs):
            worker_idx = env_idx % self.num_workers
            self.worker_env_indices[worker_idx].append(env_idx)
            self.env_to_worker_idx[env_idx] = worker_idx
    ...
    def reset_async(
        ...
        if options is not None and "reset_mask" in options:
            ...
            env_options = {key: value for key, value in options.items() if key != "reset_mask"}
            assert isinstance(
        ...
        worker_payloads: list[list[tuple[int, dict[str, Any] | None]]] = [
            [] for _ in range(self.num_workers)
        ]
        for env_idx, (env_seed, env_reset) in enumerate(zip(seed, reset_mask, strict=True)):
            if env_reset:
                env_kwargs = {"seed": env_seed, "options": env_options}
                worker_payloads[self.env_to_worker_idx[env_idx]].append(
                    (env_idx, env_kwargs)
                )
            else:
                worker_payloads[self.env_to_worker_idx[env_idx]].append((env_idx, None))

        for worker_idx, pipe in enumerate(self.parent_pipes):
            pipe.send(("reset_many", worker_payloads[worker_idx]))
    ...
    def reset_wait(
        ...
        infos: dict[str, Any] = {}
        ordered_observations: list[Any] = [None for _ in range(self.num_envs)]
        for worker_result in results:
            for env_idx, observation, info in worker_result:
                ordered_observations[env_idx] = observation
                infos = self._add_info(infos, info, env_idx)

        if not self.shared_memory:
            self.observations = concatenate(
                self.single_observation_space, ordered_observations, self.observations
            )
    ...
    def step_async(self, actions: np.ndarray):
        ...
        worker_payloads: list[list[tuple[int, Any]]] = [
            [] for _ in range(self.num_workers)
        ]
        for env_idx, action in enumerate(iter_actions):
            worker_payloads[self.env_to_worker_idx[env_idx]].append((env_idx, action))

        for worker_idx, pipe in enumerate(self.parent_pipes):
            pipe.send(("step_many", worker_payloads[worker_idx]))
    ...
    def step_wait(
        ...
        worker_results, successes = zip(*[pipe.recv() for pipe in self.parent_pipes])
        ...
        terminations = np.empty(self.num_envs, dtype=np.bool_)
        truncations = np.empty(self.num_envs, dtype=np.bool_)
        for worker_result in worker_results:
            for env_idx, observation, reward, terminated, truncated, info in worker_result:
                ordered_observations[env_idx] = observation
    ...
    def call_async(self, name: str, *args, **kwargs):
        ...
        for pipe in self.parent_pipes:
            pipe.send(("_call_many", (name, args, kwargs)))
    ...
    def call_wait(self, timeout: int | float | None = None) -> tuple[Any, ...]:
        ...
        ordered_results: list[Any] = [None for _ in range(self.num_envs)]
        for worker_result in worker_results:
            for env_idx, value in worker_result:
                ordered_results[env_idx] = value
    ...
    def set_attr(self, name: str, values: list[Any] | tuple[Any] | object):
        ...
        worker_payloads: list[list[tuple[int, tuple[str, Any]]]] = [
            [] for _ in range(self.num_workers)
        ]
        ...
        for worker_idx, pipe in enumerate(self.parent_pipes):
            pipe.send(("_setattr_many", worker_payloads[worker_idx]))
    ...
    def _check_spaces(self):
        ...
        worker_results, successes = zip(*[pipe.recv() for pipe in self.parent_pipes])
        self._raise_if_errors(successes)
        same_observation_spaces = [False for _ in range(self.num_envs)]
        same_action_spaces = [False for _ in range(self.num_envs)]
        for worker_result in worker_results:
            for env_idx, same_obs, same_action in worker_result:
                same_observation_spaces[env_idx] = same_obs
                same_action_spaces[env_idx] = same_action
...
def _async_worker_pool(
    ...
    action_spaces = {env_index: env.action_space for env_index, env in envs.items()}
    autoreset_flags = {env_index: False for env_index in envs}
    observations = {env_index: None for env_index in envs}
    ...
    try:
        while True:
            ...
            if command == "reset_many":
                reset_results: list[tuple[int, Any, dict[str, Any]]] = []
                for env_index, reset_kwargs in data:
                    if reset_kwargs is None:
                        reset_results.append((env_index, observations[env_index], {}))
                        continue

                    observation, info = envs[env_index].reset(**reset_kwargs)
                    observations[env_index] = observation
                    autoreset_flags[env_index] = False
                    if shared_memory:
                        write_to_shared_memory(
                            observation_spaces[env_index],
                            env_index,
                            observation,
                            shared_memory,
                        )
                    ...
                    reset_results.append((env_index, observation, info))

                pipe.send((reset_results, True))
            elif command == "step_many":
                ...
                for env_index, action in data:
                    ...
                    if autoreset_mode == AutoresetMode.SAME_STEP:
                        ...
                        assert autoreset_flags[env_index] is False
                        observation, reward, terminated, truncated, info = env.step(
                            action
                        )
                    else:
                    ...
                    step_results.append(
                        (env_index, observation, reward, terminated, truncated, info)
                    )

                pipe.send((step_results, True))
            elif command == "close":
                pipe.send((None, True))
                break
            elif command == "_call_many":
                ...
                call_results: list[tuple[int, Any]] = []
                for env_index, env in envs.items():
                    attr = env.get_wrapper_attr(name)
                    value = attr(*args, **kwargs) if callable(attr) else attr
                    call_results.append((env_index, value))
                pipe.send((call_results, True))
            elif command == "_setattr_many":
                for env_index, (name, value) in data:
                    envs[env_index].set_wrapper_attr(name, value)
                pipe.send((None, True))
            elif command == "_check_spaces_many":
                obs_mode, single_obs_space, single_action_space = data
                space_checks: list[tuple[int, bool, bool]] = []
                for env_index in envs:
                    same_observation_space = (
                        single_obs_space == observation_spaces[env_index]
                        if obs_mode == "same"
                    ...
                    space_checks.append(
                        (
                            env_index,
                            same_observation_space,
                            single_action_space == action_spaces[env_index],
                        )
                    )
                pipe.send((space_checks, True))
            else:
                raise RuntimeError(
                    "Received unknown command "
                    f"`{command}`. Must be one of [`reset_many`, `step_many`, `close`, `_call_many`, `_setattr_many`, `_check_spaces_many`]."
                )
    except (KeyboardInterrupt, Exception):
        error_type, error_message, _ = sys.exc_info()
        trace = traceback.format_exc()

        error_queue.put((worker_index, error_type, error_message, trace))
